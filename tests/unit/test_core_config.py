...
class TestBrowserConfig:
    ...
    def test_browser_config_invalid_dimensions_raises_error(self, width: int, height: int):
...
class TestConfig:
    ...
    def test_config_to_dict_conversion(self):
        ...
        expected_keys = {
            "browser", "webdriver", "test", "logging", 
            "reporting", "environment", "credentials", "custom"
        }
    ...
    @pytest.mark.parametrize("env_vars,expected_browser", [
        ({"PMAS_BROWSER_NAME": "firefox"}, "firefox"),
        ({"PMAS_BROWSER_NAME": "edge"}, "edge"),
        ({"PMAS_BROWSER_NAME": "chrome"}, "chrome"),
    ])
    def test_config_environment_variable_precedence(self, env_vars: Dict[str, str], expected_browser: str):
    ...
    @pytest.mark.parametrize("config_data", [
        ...
        {"environment": {"base_url": "https://test.example.com", "name": "testing"}},
    ])
    ...
    def test_config_merge_concept(self):
        ...
        override_test = TestConfig(default_timeout=5.0)
