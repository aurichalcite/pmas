...
class FakeWebDriver:
    ...
    def _setup_default_elements(self) -> None:
        ...
        self._elements_registry.update({
            "id:username": FakeWebElement("input", attributes={"type": "text", "id": "username"}),
            "id:password": FakeWebElement("input", attributes={"type": "password", "id": "password"}),
            "id:login-button": FakeWebElement("button", text="Login", attributes={"id": "login-button"}),
            "css:.login-form": FakeWebElement("form", attributes={"class": "login-form"}),
            "xpath://button[@type='submit']": FakeWebElement("button", attributes={"type": "submit"}),
        })
    ...
    def _check_failure_mode(self, operation: str) -> None:
        ...
        if operation in self._failure_modes:
            ...
            if failure_config.get("should_fail", False):
                ...
                message = failure_config.get("message", f"Simulated failure in {operation}")
    ...
    def get_last_operation(self, operation: str) -> Optional[Dict[str, Any]]:
